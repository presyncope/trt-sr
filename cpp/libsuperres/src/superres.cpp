...
static int create_frame_buffer(FrameBuffer &buf, int w, int h, sr_pixel_format fmt) {
  buf.width = w;
  buf.height = h;
  buf.format = fmt;
  ...
  if (!semi_planar)
  {
    buf.planes[2] = {base_ptr + current_offset, chroma_w_bytes, chroma_h, pitch};
  }
...
static int copy_frame_to_gpu(const sr_frame &src, const FrameBuffer &dst,
                             cudaStream_t stream) {
  if (!dst.raw_data) return -1;
  
  for (int i = 0; i < dst.num_planes; ++i) {

    const void* src_ptr= src.data[i];
    if(!src_ptr) {
      std::cerr << "[superres] Error: Source plane " << i << " is null." << std::endl;
      return -1;
    }
...
static int copy_frame_to_host(const FrameBuffer &src, sr_frame &dst,
                              cudaStream_t stream) {
  if (!src.raw_data) return -1;

  for (int i = 0; i < src.num_planes; ++i) {
    void *dst_ptr = dst.data[i];
    if(!dst_ptr) {
      std::cerr << "[superres] Error: Destination plane " << i << " is null." << std::endl;
      return -1;
    }
...
int sr_build(const sr_build_params *params) {
  ...
  if (p.min_batch_size <= 0) p.min_batch_size = 1;
  if (p.optimal_batch_size <= 0) p.optimal_batch_size = std::max(p.min_batch_size, 8);
  if (p.max_batch_size <= 0) p.max_batch_size = std::max(p.optimal_batch_size, 8);
  ...
  std::cout << "[superres] Building Model: " << p.model_onnx << "\n"
            ...
            << p.optimal_batch_size << ", " << p.max_batch_size << "]" << std::endl;
  ...
  if(!config || !parser) return -1;

  // 6. Parse ONNX
  if (!parser->parseFromFile(p.model_onnx, static_cast<int>(ILogger::Severity::kWARNING))) {
  ...
  if (!dims_match) {
    ...
    std::cerr << "  Expected: [-1, " << EXPECTED_OUT_C << ", " << EXPECTED_OUT_H << ", " << EXPECTED_OUT_W << "]" << std::endl;
    std::cerr << "  Actual:   [";
    ...
    std::cout << "[superres] Output dimension verified: [-1, 3, 512, 512]" << std::endl;
  }

  // 9. Set Memory Limits (TensorRT 10.x Update)
  if (p.max_workspace_size == 0) p.max_workspace_size = 240ULL << 20; // 256 MB default
  ...
  if (!plan) {
    std::cerr << "[superres] Error: Failed to build serialized network." << std::endl;
    return -1;
  }
...
int sr_init(sr_handle handle, const sr_init_params *params) {
  // 1. Validation
  if(!handle) { std::cerr << "[sr_init] Handle is null" << std::endl; return -1; }
  if (!params || !params->plan_file) { std::cerr << "[sr_init] Invalid parameters." << std::endl; return -1; }
  ...
  for (int i = 0; i < nb_io; ++i) {
    ...
    if (mode == TensorIOMode::kINPUT) {
      ...
      bool is_valid = (dims.nbDims == 4) &&
                      (dims.d[1] == 3) &&
                      (dims.d[2] == TILE_SIZE) &&
                      (dims.d[3] == TILE_SIZE);
      if (!is_valid) {
        std::cerr << "[sr_init] Error: Input tensor shape mismatch!" << std::endl;
        std::cerr << "  Expected: [x, 3, " << TILE_SIZE << ", " << TILE_SIZE << "]" << std::endl;
        std::cerr << "  Actual:   [";
      ...
      std::cout << "[sr_init] Found Input Tensor: " << name
                << " (Shape: [x, 3, " << TILE_SIZE << ", " << TILE_SIZE << "])" << std::endl;
    }
      ...
      if (!is_valid) {
        std::cerr << "[sr_init] Error: Output tensor shape mismatch!" << std::endl;
        std::cerr << "  Expected: [x, 3, " << TILE_SIZE * 4 << ", " << TILE_SIZE * 4 << "]" << std::endl;
        std::cerr << "  Actual:   [";
        for (int d = 0; d < dims.nbDims; ++d)
          std::cerr << dims.d[d] << (d < dims.nbDims - 1 ? ", " : "");
        std::cerr << "]" << std::endl;
      ...
      std::cout << "[sr_init] Found Output Tensor: " << name
                << " (Shape: [x, 3, " << TILE_SIZE * 4 << ", " << TILE_SIZE * 4 << "])" << std::endl;
    }
  }
  ...
  handle->model_input_width = static_cast<int>((ipar.input_width + 0.5) * ipar.prescale);
  handle->model_input_height = static_cast<int>((ipar.input_height + 0.5) * ipar.prescale);

  // Assumption: SR Model is x4 upscaling
  handle->model_output_width = handle->model_input_width * 4;   // x4 SR assumption
  handle->model_output_height = handle->model_input_height * 4;

  // Calculate Tile Grid
  int tile_size_valid = TILE_SIZE - 2 * ipar.overlap_pixels;
  handle->num_tiles_in_x = (handle->model_input_width + tile_size_valid - 1) / tile_size_valid;
  handle->num_tiles_in_y = (handle->model_input_height + tile_size_valid - 1) / tile_size_valid;
  ...
  create_frame_buffer(handle->src_yuvframe, ipar.input_width, ipar.input_height, ipar.input_format);
  create_frame_buffer(handle->dst_yuvframe, ipar.output_width, ipar.output_height, ipar.output_format);
  ...
  if (handle->need_pre_resize) {
    ...
    create_frame_buffer(handle->prescaled_yuvframe,
  ...
  void *dev_ptr = nullptr;
  handle->input_tensor.total_size = batch_size * 3 * TILE_SIZE * TILE_SIZE * input_elem_size;
  CUDA_CHECK(cudaMalloc(&dev_ptr, handle->input_tensor.total_size));
  handle->input_tensor.data.reset(dev_ptr);

  dev_ptr = nullptr;
  handle->output_tensor.total_size = batch_size * 3 * (TILE_SIZE * 4) * (TILE_SIZE * 4) * output_elem_size;
  CUDA_CHECK(cudaMalloc(&dev_ptr, handle->output_tensor.total_size));
  handle->output_tensor.data.reset(dev_ptr);

  // Bind Tensors
  handle->trt_context->setTensorAddress(handle->input_name.c_str(), handle->input_tensor.data.get());
  handle->trt_context->setTensorAddress(handle->output_name.c_str(), handle->output_tensor.data.get());

  return 0;
}

int sr_process(sr_handle handle, const sr_frame *src_frame, sr_frame *dst_frame) {
  // 1. Validation & Copy
  if (!handle) {
  ...
  if (copy_frame_to_gpu(*src_frame, handle->src_yuvframe, handle->cuda_stream.get()) != 0) {
    return -1;
  }
  ...
  if (handle->need_pre_resize) {
      resize_yuv(handle->src_yuvframe, handle->prescaled_yuvframe, ResizeMethod::BILINEAR, handle->cuda_stream.get());
      proc_src = &handle->prescaled_yuvframe;
  }

  // 3. Setup Loop Constants
  int batch_limit = (handle->ipar.concurrent_batches > 0) ? handle->ipar.concurrent_batches : 1;
  ...
  post_params.video_full_range_flag = (handle->ipar.output_color_fullrange == 1);
  ...
  // 6. Copy Back & Sync
  if (copy_frame_to_host(handle->dst_yuvframe, *dst_frame, handle->cuda_stream.get()) != 0) {
    return -1;
  }
