...
#     && cp -a TensorRT-10.15.1.29/lib/*.so* /usr/lib/x86_64-linux-gnu/ \
#     && pip install TensorRT-10.15.1.29/python/tensorrt-10.15.1.29-cp312-none-linux_x86_64.whl ;\
...
# 1. Install TensorRT C++ development libraries via APT (ABI Matching)
RUN apt-get update && apt-get install -y --no-install-recommends \
...
# 2. Install TensorRT Python bindings via pip (기존 파이썬 환경 유지 목적)
RUN pip3 install --no-cache-dir tensorrt==${TRT_VERSION}
